...
The **EsphoMaTrix** component requires an 8x32 pixel addressable_light, it is referenced by the ID `matrix_component`.
...
Force the selected screen ```icon_name``` to be displayed next. Afterwards, the loop continues from this screen. e.g., helpful for alarms. Or after an update of the value/text.
...
Awtrix and PixelIt have hard-coded functionality. EHMTX is also capable to building something like that by lambdas. But this is all your freedom.
